...
function TestLog()
{
  ...
  let oLogAttr = Log.CreateNewAttributes();
  oLogAttr.Bold = true;
  
  let nFirstFolderID = Log.CreateFolder("First folder",
    "Use folders to group similar fragments of log messages");
  let nSecondFolderID = Log.CreateFolder("Second folder", "", pmLowest, oLogAttr);
  oLogAttr.Bold = false;
  oLogAttr.Italic = true;
  let nChildFolderID = Log.CreateFolder("Another level folder", "",
  ...
  let oImage = Sys.Desktop.Picture();
